...
export class ApplicationFormComponent implements OnInit 
{
  ...
  checkEligibity:CheckEligibility = new CheckEligibility(this.customer,"",1000,"","","","","");
  data:any
  
  checkEligibilityForm = new FormGroup({
    carmaker: new FormControl(),
  ...
  onSubmit()
  {
    ...
    this.vehicle.vehicleExShowroomPrice = this.checkEligibilityForm.value.exshowroomprice;

    this.customerService.checkEligibility(this.vehicle).subscribe(data => console.log(data), error => console.log(error));
    
    this.checkEligibity.customerEmployment = this.checkEligibilityForm.value.typeofemployement;
    this.checkEligibity.existingEmi = this.checkEligibilityForm.value.existingemi;
    this.checkEligibity.annualSalary = this.checkEligibilityForm.value.yearlysalary;
    
    this.customerService.registerEligibility(this.checkEligibity).subscribe(data => console.log(data), error => console.log(error));
